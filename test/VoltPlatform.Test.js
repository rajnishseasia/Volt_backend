...
describe("VoltPlatform (UUPS)", function () {
  ...
  beforeEach(async function () {
    ...
    await usdt.deployed();
    await usdt.connect(owner).mint(user1.address, ethers.BigNumber.from("1000000").mul(ONE_USDT));
    await usdt.connect(owner).mint(owner.address, ethers.BigNumber.from("1000000").mul(ONE_USDT));
    await usdt.connect(owner).mint(user2.address, ethers.BigNumber.from("1000000").mul(ONE_USDT));
    ...
    await platform.connect(owner).depositUSDT(ethers.BigNumber.from("1000").mul(ONE_USDT));

    await platform.connect(owner).referral(user1.address, owner.address);
    await platform.connect(owner).referral(user2.address, owner.address);
    await platform.connect(user1).register();
    await platform.connect(user2).register();
  });
  ...
  it("should allow admin to withdraw surplus USDT", async function () {
    const depositAmount = ethers.BigNumber.from("10000").mul(ONE_USDT);
    await platform.connect(owner).adminDepositUSDT(depositAmount);
    const userDeposit = ethers.BigNumber.from("1000").mul(ONE_USDT);
    await platform.connect(user1).depositUSDT(userDeposit);

    const withdrawAmount = ethers.BigNumber.from("9000").mul(ONE_USDT);
    ...
    const platBal = await usdt.balanceOf(platform.address);

    expect(platBal).to.equal(
      ethers.BigNumber.from("100000").mul(ONE_USDT) 
        .add(ethers.BigNumber.from("1000").mul(ONE_USDT)) 
        .add(depositAmount)
        .add(userDeposit)
        .sub(withdrawAmount)
    );
  ...
  it("should deposit USDT and mint Volt 1:1", async function () {
    ...
    const mintedBefore = await platform.totalVoltMinted();
    ...
    expect((await platform.totalVoltMinted()).sub(mintedBefore)).to.equal(amount);
  });
  ...
  it("should lock and unlock Volt with bonus after duration", async function () {
    ...
    expect(balance).to.equal(ethers.BigNumber.from(1200).mul(ONE_USDT));
    expect(await platform.getLockedAmount(user1.address)).to.equal(0);
  });

  it("should register user with valid referrer flow", async function () {
    await platform.connect(owner).referral(addr1.address, owner.address);
    await expect(platform.connect(addr1).register())
      .to.emit(platform, "Registered").withArgs(addr1.address, owner.address);
  ...
  it("should revert registration with no preregistered referrer", async function () {
    await expect(platform.connect(addr1).register())
      .to.be.revertedWithCustomError(platform, "NoReferrerSet");
  });

  it("should allow registration after referral preregistration", async function () {
    await platform.connect(owner).referral(addr1.address, owner.address);
    await expect(platform.connect(addr1).register())
      .to.emit(platform, "Registered").withArgs(addr1.address, owner.address);
  });

  it("should revert referral with already-registered newUser", async function () {
    await expect(platform.connect(owner).referral(owner.address, owner.address))
...
it("multiple users register in a referral chain (user1 → addr1 → newUser)", async function () {

  await platform.connect(owner).referral(addr1.address, user1.address);
  await platform.connect(addr1).register();
  expect(await platform.isRegistered(addr1.address)).to.be.true;
  expect(await platform.referrerOf(addr1.address)).to.equal(user1.address);

  const newUser = (await ethers.getSigners())[4];

  await usdt.connect(owner).mint(addr1.address, ethers.BigNumber.from("100").mul(ONE_USDT));
  await usdt.connect(addr1).approve(platform.address, ethers.constants.MaxUint256);
  await platform.connect(addr1).depositUSDT(ethers.BigNumber.from("10").mul(ONE_USDT));

  await platform.connect(addr1).referral(newUser.address, addr1.address);
  await platform.connect(newUser).register();
  expect(await platform.isRegistered(newUser.address)).to.be.true;
  expect(await platform.referrerOf(newUser.address)).to.equal(addr1.address);
});


  it("should grant bonus and allow claiming vested bonus", async function () {
    const bonusAmount = ethers.BigNumber.from(100).mul(ONE_USDT);
    ...
    expect(outstandingBefore.sub(await platform.totalBonusOutstanding())).to.equal(bonusAmount);
  });
  ...
  it("should withdraw USDT with fee applied", async function () {
    ...
    const amount = ethers.BigNumber.from(600).mul(ONE_USDT);
    await platform.connect(user1).depositUSDT(amount);
    ...
    expect(after.sub(before)).to.equal(ethers.BigNumber.from(570).mul(ONE_USDT));
    expect(await volt.balanceOf(user1.address)).to.equal(0);
    expect(await platform.totalVoltBurned()).to.equal(amount);
  });

  it("should payout referral bonuses (vested)", async function () {
    const amount = ethers.BigNumber.from(1000).mul(ONE_USDT);
  ...
  it("reverts deposit for unregistered user", async function () {
    await expect(platform.connect(addr1).depositUSDT(ethers.BigNumber.from(1000).mul(ONE_USDT)))
      .to.be.revertedWithCustomError(platform, "OnlyActiveRegistered");
  });
  ...
  it("handles multiple locks", async function () {
    const amount1 = ethers.BigNumber.from(1000).mul(ONE_USDT);
    const amount2 = ethers.BigNumber.from(500).mul(ONE_USDT);
  ...
  it("should handle lock for 45-day duration with multiplier and interest", async function () {
      const depositAmount = ethers.BigNumber.from("40").mul(ONE_USDT);
      await platform.connect(user1).depositUSDT(depositAmount);
      const lockAmount = ethers.BigNumber.from("20").mul(ONE_USDT);
      ...
      const availableInterest = depositAmount.sub(lockAmount).mul(600).mul(45 * 24 * 60 * 60)
        .div(10000).div(365 * 24 * 60 * 60);

      expect(interestDue).to.be.closeTo(lockInterest.add(availableInterest), 1e6);
  ...
  it("should handle all lock durations with correct multipliers and interest", async function () {
      const depositAmount = ethers.BigNumber.from("25").mul(ONE_USDT); // exact sum of 5 locks of 5
      await platform.connect(user1).depositUSDT(depositAmount);
      const lockAmount = ethers.BigNumber.from("5").mul(ONE_USDT);

      const durations = [45, 90, 180, 365, 1095];
      const aprs = [150, 350, 800, 1800, 10000];

      for (let i = 0; i < durations.length; i++) {
        await platform.connect(user1).lock(lockAmount, durations[i]);
      }

      await ethers.provider.send("evm_increaseTime", [1095 * 24 * 60 * 60]);
      await ethers.provider.send("evm_mine");
  ...
  it("should revert unlocking with invalid lock index", async function () {
    ...
    await expect(platform.connect(user1).unlock(1)).to.be.revertedWith("Index");
    await expect(platform.connect(user1).unlock(100)).to.be.revertedWith("Index");
  ...
  it("reverts premature unlock", async function () {
    ...
    await ethers.provider.send("evm_mine");
    await expect(platform.connect(user1).unlock(0)).to.be.revertedWith("Still locked");
  });
  ...
  it("reverts withdrawal below minimum", async function () {
    ...
    await platform.connect(user1).depositUSDT(amount);
    await expect(platform.connect(user1).withdrawUSDT(amount, false)).to.be.revertedWith("Below min");
  });
  ...
  it("should revert lock with insufficient Volt balance", async function () {
    ...
    await platform.connect(user1).depositUSDT(depositAmount);
    const lockAmount = ethers.BigNumber.from("200").mul(ONE_USDT);
    await expect(platform.connect(user1).lock(lockAmount, 45))
  ...
  it("should revert multiple locks exceeding balance", async function () {
    const depositAmount = ethers.BigNumber.from("500").mul(ONE_USDT);
    await platform.connect(user1).depositUSDT(depositAmount);
    const lockAmount = ethers.BigNumber.from("300").mul(ONE_USDT);

    await platform.connect(user1).lock(lockAmount, 45);
    expect(await platform.getLockedAmount(user1.address)).to.equal(lockAmount);
    expect(await platform.balanceOfVolt(user1.address)).to.equal(depositAmount.sub(lockAmount));

    const excessLockAmount = ethers.BigNumber.from("300").mul(ONE_USDT);
  ...
  it("should revert zero-amount lock", async function () {
    const depositAmount = ethers.BigNumber.from("1000").mul(ONE_USDT);
    await platform.connect(user1).depositUSDT(depositAmount);
    await expect(platform.connect(user1).lock(0, 45)).to.be.revertedWith("Amount=0");
  });
  ...
  it("allows parameter updates by owner", async function () {
    await platform.connect(owner).updateParams(700, 200 * ONE_USDT, 1200, 600, [2000, 1000, 400, 200, 100, 50, 20]);
    expect(await platform.baseApyBp()).to.equal(700);
    expect(await platform.minWithdrawUSDT()).to.equal(200 * ONE_USDT);
    ...
    await expect(platform.connect(user1).updateParams(600, 150 * ONE_USDT, 1000, 500, [1000, 500, 200, 100, 50, 25, 10]))
      .to.be.revertedWithCustomError(platform, "OwnableUnauthorizedAccount");
  });

  it("should prevent admin withdraw below liability (surplus)", async function () {
    await platform.connect(owner).adminDepositUSDT(ethers.BigNumber.from(1000).mul(ONE_USDT));
    await platform.connect(user1).depositUSDT(ethers.BigNumber.from(800).mul(ONE_USDT));
    const bal = await usdt.balanceOf(platform.address);
    const liability = (await platform.totalVoltMinted()).sub(await platform.totalVoltBurned());
    const surplus = bal.sub(liability);
    const tooMuch = surplus.add(1);

    await expect(platform.connect(owner).adminWithdrawUSDT(tooMuch))
      .to.be.revertedWith("Exceeds surplus");
  });

  it("should handle full exit withdrawal", async function () {
    await platform.connect(owner).adminDepositUSDT(ethers.BigNumber.from("10000").mul(ONE_USDT));
    const depositAmount = ethers.BigNumber.from("600").mul(ONE_USDT);
    await platform.connect(user1).depositUSDT(depositAmount);
  ...
  it("should revert parameter updates with invalid values", async function () {
    await expect(platform.connect(owner).updateParams(10001, 150 * ONE_USDT, 1000, 500, [1000, 500, 200, 100, 50, 25, 10]))
      .to.be.revertedWith("Invalid APY");

    await expect(platform.connect(owner).updateParams(600, 0, 1000, 500, [1000, 500, 200, 100, 50, 25, 10]))
      .to.be.revertedWith("Invalid min withdraw");

    await expect(platform.connect(owner).updateParams(600, 150 * ONE_USDT, 5001, 500, [1000, 500, 200, 100, 50, 25, 10]))
      .to.be.revertedWith("Invalid fees");

    await expect(platform.connect(owner).updateParams(600, 150 * ONE_USDT, 1000, 500, [5001, 500, 200, 100, 50, 25, 10]))
